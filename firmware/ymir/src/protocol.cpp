...
static int handle_resume_query(transport_t& t, const std::byte* payload, uint16_t plen)
{
    (void)payload;
    if (plen < 1) {
        return send_nak(t, NAK_BAD_STATE);
    }
    // No per-chunk metadata updates, so no byte-level resume
    std::array<uint8_t, 4> resp{};
    return send_frame(t, CMD_RESUME_QUERY, resp.data(), 4);
}
...
int protocol_run(transport_t& t)
{
    ...
    for (;;) {
        ...
        case CMD_RESUME_QUERY:
            handle_resume_query(t, payload.data(), static_cast<uint16_t>(payload.size()));
            break;
        default:
            send_nak(t, NAK_BAD_STATE);
            break;
