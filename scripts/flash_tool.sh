...
function do_flash() {
    ...
    send_frame $CMD_RESUME_QUERY "$slot"
    recv_frame 10 || die "no RESUME_QUERY response"
    local -i resume=0
    if (( RECV_CMD == CMD_RESUME_QUERY && ${#RECV_PAYLOAD[@]} >= 4 )); then
        resume=$(( (RECV_PAYLOAD[0]) |
                   ((RECV_PAYLOAD[1]) << 8) |
                   ((RECV_PAYLOAD[2]) << 16) |
                   ((RECV_PAYLOAD[3]) << 24) ))
    fi
